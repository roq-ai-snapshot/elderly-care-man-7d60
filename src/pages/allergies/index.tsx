...
import { getAllergies, deleteAllergyById } from 'apiSdk/allergies';
...
function AllergyListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('allergy', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/allergies/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('allergy', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Allergy
            </Text>
            <NextLink href={`/allergies/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('allergy', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/allergies/edit/${record.id}`} passHref legacyBehavior>
                          <Button
