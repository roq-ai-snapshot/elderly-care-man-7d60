...
import { getCaregivers, deleteCaregiverById } from 'apiSdk/caregivers';
...
function CaregiverListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('caregiver', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/caregivers/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('caregiver', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Caregiver
            </Text>
            <NextLink href={`/caregivers/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('caregiver', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/caregivers/edit/${record.id}`} passHref legacyBehavior>
                          <Button
