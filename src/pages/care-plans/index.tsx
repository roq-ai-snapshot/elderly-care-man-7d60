...
import { getCarePlans, deleteCarePlanById } from 'apiSdk/care-plans';
...
function CarePlanListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('care_plan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/care-plans/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('care_plan', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Care Plan
            </Text>
            <NextLink href={`/care-plans/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('care_plan', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/care-plans/edit/${record.id}`} passHref legacyBehavior>
                          <Button
