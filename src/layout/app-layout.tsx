...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'care-nurse', 'doctor', 'staff'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<CareHomeInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/care-homes` : null),
    () => getCareHomes({ tenant_id: session?.user?.tenantId }),
  );
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Care Home',
      path: isTenantUser ? `/care-homes/view/${data?.[0]?.id}` : '/care-homes',
      entity: 'care_home',
      service: AccessServiceEnum.PROJECT,
      icon: FiHome,
    },
    { name: 'Allergy', path: '/allergies', entity: 'allergy', service: AccessServiceEnum.PROJECT, icon: FiAlertCircle },
    {
      name: 'Appointment',
      path: '/appointments',
      entity: 'appointment',
    ...
    { name: 'Medication', path: '/medications', entity: 'medication', service: AccessServiceEnum.PROJECT, icon: FiBox },
    { name: 'Resident', path: '/residents', entity: 'resident', service: AccessServiceEnum.PROJECT, icon: FiUserCheck },
